...
export const BLOCKING = new Set([
  ...
  CODES.NO_SUCH_SEED,
  CODES.SEED_FAILED,
  // UNHEALTHY blocks because the alternative is worse than stopping: a server
  // that answers with the wrong thing drives a walk that fails deep inside the
  // recording, and the video then looks like the feature is broken.
  CODES.UNHEALTHY,
])
