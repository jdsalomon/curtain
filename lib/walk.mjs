...
export async function walk(name, { cwd = process.cwd(), force = false, gif = true, io = {} } = {}) {
  ...
  // 5. Artifacts. One directory per walk under this workspace's state dir, wiped
  ...
  // Playwright waits 30s for a missing element, which is right for a CI suite and
  // wrong for a recorder: `up` already waited for the app to be ready, so anything
  // this walk cannot find in ten seconds is absent rather than slow, and a demo
  // that is going to fail should fail while someone is still watching. One default
  // on the context covers every operation, including waitFor and goto.
  ...
  return { ...base, browser: from, env, passed, exitCode: passed ? 0 : 1 }
}
